...
public class QuickSort implements Sort {
    ...
    public void quickSort(int[] list, int low, int high) {
        if (low < high) {
            //将list数组进行一分为二,找到中轴的位置
            int middle = getMiddle(list, low, high);
            //对低字表(即小于中轴的一边)进行递归排序
    ...
    public int getMiddle(int[] list, int low, int high) {
        //将list[low]中的数保存到temp中，可以理解成在数组list[low]上挖了个坑,数组的第一个作为中轴【坑】
        int temp = list[low];
        while (low < high) {
            // 从右向左找第一个小于 temp 的数 去【填坑】 (试着把while 循环转成 for形式的)
            while (low < high && list[high] > temp) {
                high--;//(正常情况 )
            }
            //把异常的list[high] 比中轴小的记录 移到低端 list[low], 同时产生新坑即中轴list[higt]
            list[low] = list[high];
        ...
        //退出时，low等于high。将temp填到这个坑中，中轴记录到尾
        list[low] = temp;
        //返回中轴的位置
        return low;
    }
